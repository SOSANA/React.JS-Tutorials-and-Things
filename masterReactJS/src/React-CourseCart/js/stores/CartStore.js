...
var _courses = {}, _cartVisible = false;

// Add product to cart
function add(CourseID, update) {
  update.quantity =  1;
  _courses[CourseID] = _.extend({}, _courses[CourseID], update)
}
...
var CartStore = _.extend({}, EventEmitter.prototype, {
  ...
  getCartTotal: function() {
    var total = 0;
    for(product in _courses){
      if(_courses.hasOwnProperty(product)){
        total += _courses[product].price * _courses[product].quantity;
      }
    }
...
AppDispatcher.register(function(payload) {
  ...
  switch(action.actionType) {
    ...
    case FluxCartConstants.CART_ADD:
      add(action.CourseID, action.update);
      break;
